...
- `bakignore` → 备份时忽略的目录后缀集合（系统自动维护，与 `baksuffix` 联动，仅 backup 生效）
- `kmmignore` → 原地忽略规则集合（来自 `.kmmignore` 文件，gitignore 语法，由 Planner 现场读取，不搬动不拷贝，apply/backup/restore 均生效，详见 DESIGN_KMMIGNORE_RULES.md）
