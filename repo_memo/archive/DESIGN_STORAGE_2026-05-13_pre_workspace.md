...
- 扫描结束后，所有 game 和 mod 条目写入 database，此时可能存在同名 appid / mixed_id 的多条记录，无重复 appid/mixed_id 的条目自动设为 `managed: true`；仅重复条目初始为 `false` 待用户决策
...
| `modmanager:datasource-db` | `stores/datasource.ts` | 【冗余，待 TODO-28 移除】扫描结果的独立缓存：libraries、games、mods、lastResult |
...
- **单副本原则**：database 的完整 JSON 在前端仅有一份权威副本，禁止同一份数据写入多个不同的 key。当前 `datasource`、`datasource-db`、`forest-store` 三个 key 中均包含 database 数据，为历史遗留问题（见 TODO-28），合并后仅保留一个 key
...
| 数据库扫描结果 | 后端更新时同步刷新 | 扫描完成/保存 managed 后，前端从 API 响应中获取最新 database 并覆盖本地副本。管道消费前由 validate_database 过滤非 managed 条目（向后兼容：若整个数据库无任何 managed=true，则全部视为 managed 处理） |
...
**权威性**：前端任何时候只持有一份 database 副本。任何页面/组件需要 database 数据时，必须从该唯一副本读取，不得创建独立缓存。副本的更新路径唯一：后端 API 响应 → 覆盖 store 中的单例副本 → 写入 localStorage（单 key）。设置页、数据源页等所有 database 修改入口，均通过"修改→传后端→取回→覆盖本地"闭环完成，不存在前端本地修改后不同步的情况。
...
| D4 | 扫描缓存独立于 database | 【已废弃】通过 managed 字段在 database.json 内实现去重，无需独立缓存文件（见 §6） |
