...
`apiGet` / `apiPost` / `streamSse` 在发出请求前，从 sessionStorage（优先）→ localStorage（回退）读取 `configIndex`，序列化为 JSON 字符串，设为 `X-UserConfig-Index` header。

若两个存储都没有 `configIndex`，返回错误（与当前行为一致）。
