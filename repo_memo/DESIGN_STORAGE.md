...
| key | 内容 |
|-----|------|
| `modmanager:workspace` | 包含 `lastDatabase`、`perDatabase`（按 name 索引的 decisions/results）、`aggregatedRuleMeta`（output_path/hash/time/paths） |
...
| `modmanager:workspace` | 聚合 workspace 对象（含 lastDatabase、perDatabase、aggregatedRuleMeta） | ComputePrepPage / ConflictsPage / DatabaseSelector |
