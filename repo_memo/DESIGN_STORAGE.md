...
默认位置仅用于前端在双空存储时向 `/api/os/defaults` 请求初值；后端业务流程不执行 `config_index` 默认路径猜测。
...
| 来源 | 触发条件 | 行为 |
|------|----------|------|
| **显式传入** | Orchestrator / CLI / Web API 从 Header 解出 `config_index` | 直接使用该路径 |

### 3.2 默认值来源（前端使用）

| 平台 | 默认 `config_index.string` |
|------|------|
| Linux | `~/.config/kmm/user_config.json` |
| Windows | `%appdata%/kmm/user_config.json` |

该默认值通过 `/api/os/defaults` 提供给前端。前端写穿到 sessionStorage/localStorage 后，再经 `X-UserConfig-Index` 传入后端。

### 3.3 自动生成（缺失即创建）

```
使用传入的 config_index 路径
  ├── 文件存在 → 加载并返回
  └── 文件不存在
        ├── 尝试在该路径创建空默认 user_config.json
        │     ├── 写入成功 → 返回空配置
        │     └── 写入失败 → 报错：
...
Orchestrator 向 bootstrap 传递 `config_index`：直接使用传入路径。Web 路径下该值由 `X-UserConfig-Index` 解析得到。
