...
> 创建：2026-05-13
> 依赖：DESIGN_GUI.md（页面流）、DESIGN_WORKSPACE_STATE.md（结果缓存）、DESIGN_GUI_DATASOURCE_TAB.md（数据源展示）
...
| 元素 | 初始状态 | 行为 |
|------|---------|------|
| [▶ 开始计算] | 始终可点击 | 构建 managed_entries → `POST /api/pipeline/compute` |
| [查看结果] | 灰色（disabled） | workspace.results 非空时亮起可点击 → 跳转 `/forest` |
| 提示文字 | 动态计算 | "覆盖 N 个游戏 (X 个有多个入口) / 覆盖 N 个 MOD (Y 个有多个入口)" |

**按钮状态**：[查看结果] 不依赖 [▶ 开始计算] 的触发——它只看 `GET /api/workspace/status` 的 `results.timestamp` 是否为 null。这样两个按钮之间无耦合。
...
```
GET /api/workspace/status → inputs.aggregated_rule_path
  → POST /api/rules/affected-entries { aggregated_rule_path }
...
{
  "database_path": "/tmp/.../database.json",   // 从 workspace.inputs 读取
  "rule_paths": ["/home/.../rules/"],           // 从 workspace.inputs 读取
  "user_config_path": "~/.config/kmm/...",      // 从 workspace.inputs 读取
  "branch_decisions": { ... },                  // 从 workspace.decisions 读取（合并）
  "managed_entries": { ... }                    // 从 checkbox 状态构造（可选）
}
```
...
compute SSE 流结束 → onResult:
  成功 →
    1. POST /api/workspace/save-results { trees_count, mapping_count, warnings, errors, stats, inputs_hash }
    2. [查看结果] 按钮亮起（下一次 GET /api/workspace/status 时 results.timestamp 非 null）
...
```
RulesOverview [保存规则选择]
  → POST /api/rules/aggregate → aggregated_rule_set.json
  → POST /api/workspace/save-inputs { aggregated_rule_path }

计算准备页 mount
  → GET /api/workspace/status → inputs.aggregated_rule_path
  → POST /api/rules/affected-entries → 返回被引用条目列表（含重复标记）
  → 渲染表格

用户取消 checkbox → Pinia ref 更新 → 顶部提示文字实时更新

用户点 [▶ 开始计算]
  → 收集 checkbox 状态 → managed_entries
  → POST /api/pipeline/compute { ..., managed_entries }
  → 成功 → save-results → [查看结果] 亮起
...
| workspace.results 为空 | [查看结果] 灰色 disabled |
| compute 中途关闭页面 | workspace.results 不更新（只有成功后调 save-results） |
...
| 文件 | 职责 |
|------|------|
| `frontend/src/pages/ComputePrepPage.vue` | 页面组件 |
| `src/modmanager_web/routes/rules.py`（改） | 新增 `POST /api/rules/affected-entries` |
| `src/modmanager_web/schemas.py`（改） | `ComputeRequest` 新增 `managed_entries` 字段 |
...
| # | 决策 | 结论 |
|---|------|------|
| D1 | managed_entries 存储 | 不在 workspace 中——作为 compute 参数直接传入 |
| D2 | 默认 checkbox 状态 | 全选（不干预 = 全部留给 engine） |
| D3 | 重复条目视觉提示 | 柔和高亮，非刺眼 |
| D4 | compute 后行为 | 不自动跳转——用户主动点 [查看结果] |
| D5 | [查看结果] 按钮状态 | 依 workspace.results.timestamp 是否非 null 决定 |
