...
**请求体：无**。所有参数从工作区目录读取：
- `aggregated_rule_set` → `{workspace_dir}/{id}/aggregated_rule.json`
- `database_name` → `{workspace_dir}/{id}/meta.json` 的 `database_name` 字段
- `managed_entries` / `branch_decisions` → `{workspace_dir}/{id}/decisions.json`

前端调用时仅需传入 workspace_id，无需携带任何业务参数。
...
用户点 [▶ 开始计算]
  ...
  → POST /api/workspace/{id}/pipeline/compute（无请求体——参数从工作区读取）
  → orchestrator 内部从工作区获取 database / aggregated_rule_set / decisions
...
| # | 决策 | 结论 |
|---|------|------|
| D1 | managed_entries 存储 | 后端工作区 `decisions.json` 为权威 |
