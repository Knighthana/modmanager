...
| ConflictsPage | `/conflicts` | 分支冲突表格 + radio 选择；[确认决策] → 写 branch_decisions 到 localStorage | 写 `localStorage.modmanager:decisions:{name}.branch_decisions` |
...
**[▶ 开始计算]**：
- 从 localStorage 读 `decisions:{database_name}` 获取历史决策
- 收集 checkbox 状态 → 构建 managed_entries（仅包含被干预的条目）
- `POST /api/pipeline/compute { database_name, managed_entries, branch_decisions, ... }`
- 成功后 → 前端写 `results:{database_name}` 到 localStorage → [查看结果] 按钮亮起
- 失败 → 错误信息在此页展示
...
| 📐 小地图 | toggle 小地图显示/隐藏 |
| 📊 摘要 (n⚠) | toggle 右侧抽屉；n 为 `localStorage.results:{name}.warnings` 计数 badge |

#### 结果有效性

- Forest 加载时读 `localStorage.modmanager:results:{name}` → 读取 `inputs_hash` 与 `timestamp`
...
- 用户通过 radio 选择候选
- [确认决策] 按钮 → 写 `localStorage.modmanager:decisions:{name}.branch_decisions`
...
| 用户决策（decisions, results） | 前端 localStorage（`modmanager:decisions:{name}` / `modmanager:results:{name}`） |
...
- 刷新恢复：读 `localStorage.modmanager:lastDatabase` 恢复选中
...
| D2 | managed 归属 | 前端 localStorage（`modmanager:decisions:{name}.managed_entries`），compute 时作为参数传入 |
...
| D8 | branch_decisions 保存 | ConflictsPage [确认决策] 按钮 → 写 localStorage.modmanager:decisions:{name}.branch_decisions |
