...
| 📐 小地图 | toggle 小地图显示/隐藏 |
| 📊 摘要 (n⚠) | toggle 右侧抽屉；n 为 `workspace.perDatabase[name].results.warnings` 计数 badge |

#### 结果有效性

- Forest 加载时读 `localStorage.modmanager:workspace` 中 `perDatabase[name].results` → 读取 `inputs_hash` 与 `timestamp`
