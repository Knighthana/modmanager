...
- apply 由 `dispatch()` 在 APPLY intent 下通过四层管线调用，不作为工作区入口本身。
- 前置门禁（preflight / gate check）为 Planner 层职责：`plan_fileops()` 内调用 preflight，
  `dispatch()` 仅在 manifest.ok 时继续执行 apply 原语 `apply_entries()`。
