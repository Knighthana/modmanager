...
- prep 负责创建 `backup_dir` 与初始化 `backupinfo.json`/tree。backup 原语负责消费已准备状态并执行差异备份。结构定义统一以 `DESIGN_BACKUP_DIR.md` 为准。
...
当被 backup 的目录中没有 backup_dir 时，由 prep 阶段建立 backup_dir、创建 `backupinfo.json` 并完成初始建树。

**建树**：在 ignore 的基础上，迭代扫描源 `mixed_id`（base 或者 mod）目录中**所有**的目录和文件放入树上的相应结点。所有文件的 hash 对象的 `hashtype` 标记为 `"invalid"`，`hashvalue` 标记为 `"0"`。

prep 建树完成后，backup 原语读取现有 tree 进入正常 backup 工作。
...
当 backup_dir 不存在时：prep 阶段完成创建 backup_dir → 创建 `backupinfo.json` → 立即建树（扫描源目录，所有文件标记 `isbackuped=false`，hash `hashtype="invalid"` `hashvalue="0"`）→ 写入 `backupinfo.json`。随后 backup 原语按 §六 执行备份操作。
...
当 backup_dir 存在但 tree 为空（`{}` 占位符）或不完整时：由 prep 阶段视为首次备份，重新执行 §五.2 的建树操作；backup 原语在 tree 就绪后再执行。
...
- **首次备份**（backup_dir 不存在或 tree 为空）：prep 阶段在文件复制之前执行建树。扫描源目录（`backup_dir` 的父目录，即 content_root）生成完整文件结构镜像，写入 `backupinfo.json`。backup 原语只消费该已初始化 tree。
- **后续备份**（tree 已存在且完整）：不重建树。将现有树读入内存，按 §六 逐个更新结点状态，每次更新后写回 `backupinfo.json`。
- **dry_run 模式**：prep 与 backup 原语均不得产生磁盘写操作；仅返回结构化预测结果。
