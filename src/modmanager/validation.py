...
def validate_aggregated_rule_set(aggregated_rule_set: Any) -> list[str]:
    ...
    2. aggregated_rule_set['operation'] must be a list
    3. Each operation entry must have 'mixed_id' as string
    4. All mixed_id must be unique
    5. All mixed_id must be in appid:modid format
    6. actionlist items must have explicit 'action' and 'destin' as strings
    7. 'destin' must be appid:modid format or 'none' when not 'hold'
    """
    errors: list[str] = []
    ...
    if "operation" not in aggregated_rule_set:
        return [f"E_AGGREGATED_RULE_SET_INVALID: aggregated_rule_set missing 'operation' key"]

    operations = aggregated_rule_set["operation"]
    if not isinstance(operations, list):
        return [f"E_AGGREGATED_RULE_SET_INVALID: aggregated_rule_set['operation'] must be list, got {type(operations).__name__}"]
    ...
    for idx, op_obj in enumerate(operations):
        if not isinstance(op_obj, dict):
            errors.append(f"E_AGGREGATED_RULE_SET_INVALID: aggregated_rule_set['operation'][{idx}] must be dict, got {type(op_obj).__name__}")
            continue

        # Check mixed_id
        mixed_id = op_obj.get("mixed_id")
        if not isinstance(mixed_id, str):
            errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['mixed_id'] must be string, got {type(mixed_id).__name__ if mixed_id is not None else 'null'}")
            continue

        if not mixed_id:
            errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['mixed_id'] cannot be empty string")
            continue

        # Check mixed_id format
        if split_mixed_id(mixed_id) is None:
            errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['mixed_id'] {mixed_id!r} must be appid:modid format")
            continue

        # Check uniqueness
        if mixed_id in seen_mixed_ids:
            errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['mixed_id'] {mixed_id!r} is not unique")
            continue
        ...
        if not isinstance(actionlist, list):
            errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'] must be list, got {type(actionlist).__name__}")
            continue

        for item_idx, item in enumerate(actionlist):
            if not isinstance(item, dict):
                errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}] must be dict, got {type(item).__name__}")
                continue

            # action and destin are required (no fallback to def_action/def_destin)
            action = item.get("action", "")
            if action == "hold":
                continue

            if not isinstance(action, str) or not action:
                errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}] 'action' must be a non-empty string")
                continue

            destin_item = item.get("destin")
            if not isinstance(destin_item, str):
                errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}] 'destin' must be a string")
                continue

            if not _is_none_destin(destin_item) and split_mixed_id(destin_item) is None:
                errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}]['destin'] {destin_item!r} must be appid:modid format or 'none'")
            ...
            if action != "delete":
                ...
                if not isinstance(from_list, list) or not from_list or not all(isinstance(f, str) for f in from_list):
                    errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}] action={action!r} requires 'from' list[string] field")
            
            into_list = item.get("into")
            if not isinstance(into_list, list) or not into_list or not all(isinstance(t, str) for t in into_list):
                errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}] action={action!r} requires 'into' list[string] field")
            ...
            if action != "delete":
                ...
                if (from_is_multi or from_has_glob) and into_is_multi:
                    errors.append(f"E_AGGREGATED_RULE_SET_INVALID: operation[{idx}]['actionlist'][{item_idx}] forbids multi-source to multi-target in single action")
