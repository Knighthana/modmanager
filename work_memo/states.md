...
- [ ] TODO-20: （挂起）选项卡解耦——各选项卡独立设计，避免互相影响
- [ ] TODO-21: （挂起）"发现模式"偏好保存应在"扫描 Steam 库"按钮按下后执行前端持久化
- [ ] TODO-22: （挂起）Database 路径 / Database JSON / Rule paths 迁入"数据源"选项卡
- [ ] TODO-23: （挂起）user config 路径迁入"设置"；启动时未配置则导航到设置页并提示填写；已配置则前往用户偏好的"默认开始页"
- [ ] TODO-24: （挂起）backup dir 语义澄清——backup dir 由 `build_backup_dir()` 自动推导（见 DESIGN_BACKUP.md），非用户自由填写；当前 ForestPage 上的 backup_dir 输入框需审查
- [ ] TODO-25: （挂起）"Forest 可视化"中的 dry run 和"应用流水线"功能迁入"文件操作"选项卡
