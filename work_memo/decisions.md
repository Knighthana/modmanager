...
- **P5**: `.kmmignore`（gitignore 语法）由 Planner 统一收集，仅来自 `.kmmignore` 文件（gitignore 模型），对所有操作生效
...
- **P10**: `bakignore` 独立于 `kmmignore`——bakignore 联动 `baksuffix` 自动维护，仅 backup 生效；kmmignore 仅来自 `.kmmignore` 文件，gitignore-parser 匹配，全部操作生效
