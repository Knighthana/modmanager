...
class TestListSources:
    ...
    def test_list_sources_returns_all_keys(
        ...
        resp = client.post("/api/rules/list-sources", json={}, headers={"X-UserConfig-Index": '{"type":"path","string":"/fake/config.json"}'})
...
class TestScanBySource:
    ...
    def test_scan_known_source_with_directories(
        ...
        resp = client.post(
            ...
            headers={"X-UserConfig-Index": '{"type":"path","string":"/fake/config.json"}'},
        )
        assert resp.status_code == 200
        body = resp.json()
        assert body["ok"] is True
